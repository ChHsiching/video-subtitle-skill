...
- **Chinese cue length ≤ 56 width units** (CJK char = 2, ASCII char = 1; ≈ 28 Chinese characters). This is a display-width limit, not a character count — a mixed cue like "agent 的区分" measures wider than its character count suggests. The `shorten` step splits long cues at punctuation so each resulting cue is one line. But the floor matters just as much: no cue should be a bare word or punctuation. If you can't fill a cue with at least a short complete phrase, the cue shouldn't exist as a standalone — fold it in.
